  ...
  useEffect(() => {
    const fetchVehicleData = async () => {
      try {
        ...
        if (devices.length > 0) {
          const device = devices[0]; // Get device info from devices array
          const dataArray = await getDeviceData(device.id);
          const data = dataArray[0]; // Get first data point
          ...
          if (data) {
            setVehicleData({
              ...
              year: device.year || 0,
              odometer: data.speed * 1000 || 0, // Mock odometer calculation
