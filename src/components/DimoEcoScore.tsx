    ...
    const fetchEcoScoreData = async () => {
      try {
        ...
        if (devices.length > 0) {
          ...
          const dataArray = await getDeviceData(deviceId);
          const tripsArray = await getDeviceTrips(deviceId);
          
          // Get the first data point and trip
          const data = dataArray[0];
          const trips = tripsArray;
          
          if (data && trips.length > 0) {
            // Calculate eco-score based on DIMO data
            const fuelEfficiency = data.fuelLevel || 0;
            const ecoFriendlyTrips = trips.filter((trip: any) => trip.ecoScore > 80).length || 0;
            const score = Math.min(100, Math.round((fuelEfficiency * 0.5) + (ecoFriendlyTrips * 10)));
