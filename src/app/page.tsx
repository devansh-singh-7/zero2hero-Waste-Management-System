  ...
  useEffect(() => {
    async function fetchImpactData() {
      try {
        ...
        const [reportsResponse, rewardsResponse, tasksResponse] = await Promise.all([
          fetch('/api/reports?type=recent&limit=100'),
          fetch('/api/rewards?type=all'),
          fetch('/api/reports?type=tasks&limit=100')
        ]);
        ...
        const reports = Array.isArray(reportsJson) ? reportsJson : [];
        const rewards = Array.isArray(rewardsJson) ? rewardsJson : [];
