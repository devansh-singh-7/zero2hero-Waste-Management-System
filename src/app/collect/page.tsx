...
export default function CollectPage() {
  ...
  const handleStatusChange = async (taskId: number, newStatus: CollectionTask['status']) => {
    ...
    try {
      const response = await fetch(`/api/waste-collection-tasks/${taskId}/status`, {
        method: 'PUT',
        headers: {
      ...
      try {
        ...
        if (parsedResult.wasteTypeMatch && parsedResult.quantityMatch && parsedResult.confidence > 0.7) {
          ...
          const rewardResponse = await fetch(`/api/rewards`, {
            ...
            body: JSON.stringify({ userId: user.id, amount: earnedReward }),
          })
          if (rewardResponse.ok) {
            const rewardData = await rewardResponse.json()
            // setReward(earnedReward) // This state is not used in the current UI, but keeping it for now
          ...
          const collectedWasteResponse = await fetch(`/api/collected-waste`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({ taskId: selectedTask.id, userId: user.id, verificationResult: parsedResult }),
          })
          if (collectedWasteResponse.ok) {
            const collectedWasteData = await collectedWasteResponse.json()
            // setReward(earnedReward) // This state is not used in the current UI, but keeping it for now
            toast.success(`Collection verified and waste saved!`, {
              duration: 5000,
              position: 'top-center',
            })
          } else {
            toast.error('Failed to save collected waste. Please try again.')
          }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      ...
      <div className="mb-4">
        ...
        <LocationPicker onLocationSelect={(loc) => setSelectedLocation(loc)} />
      </div>
